...
from .models import EType, EUniverse, EInstanceFlag, ETypeChar

ETypeChars = ''.join(ETypeChar.__members__.keys())


class SteamID(int):
    """Convert a steamID to its various representations"""
...
async def steam64_from_url(url, http_timeout=30):
    ...
    try:
        ...
        if match.group('type') in ('id', 'profiles'):
            async with _session.get(match.group('clean_url'), timeout=http_timeout) as r:
                text = await r.text()
